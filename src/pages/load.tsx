          ...
          <main className="py-8">
            <div className="text-center mb-8">
              <h1 className="text-4xl md:text-5xl font-bold text-gray-900 mb-4">
                📱 {currentLanguage === 'ko' ? '선불 충전' :
                     currentLanguage === 'tl' ? 'Load' :
                     'Load/Prepaid'}
              </h1>
              <p className="text-xl text-gray-600 mb-6">
                {currentLanguage === 'ko' ? '모든 통신사 선불 충전을 빠르고 편리하게' :
                 currentLanguage === 'tl' ? 'Mabilis at convenient na load para sa lahat ng network' :
            ...
            <div className="mb-8">
              <h2 className="text-2xl font-bold text-gray-900 mb-4">
                {currentLanguage === 'ko' ? '통신사 선택' :
                 currentLanguage === 'tl' ? 'Piliin ang Network' :
                 'Select Network'}
              </h2>
            ...
            <div className="mb-8">
              <h2 className="text-2xl font-bold text-gray-900 mb-4">
                {currentLanguage === 'ko' ? '충전 금액' :
                 currentLanguage === 'tl' ? 'Halaga ng Load' :
                 'Load Amount'}
              </h2>
            ...
            <div className="bg-white rounded-xl p-6 shadow-sm border border-gray-200">
              <h3 className="text-lg font-semibold text-gray-900 mb-4">
                {currentLanguage === 'ko' ? '충전 정보 입력' :
                 currentLanguage === 'tl' ? 'I-enter ang Details' :
                 'Enter Load Details'}
              </h3>
              <div className="space-y-4">
                <div>
                  <label className="block text-sm font-medium text-gray-700 mb-2">
                    {currentLanguage === 'ko' ? '휴대폰 번호' :
                     currentLanguage === 'tl' ? 'Mobile Number' :
                     'Mobile Number'}
                  </label>
                  <input
                    type="tel"
                ...
                <button className="w-full px-6 py-3 bg-blue-600 text-white rounded-lg hover:bg-blue-700 transition-colors font-semibold">
                  {currentLanguage === 'ko' ? '충전하기' :
                   currentLanguage === 'tl' ? 'Mag-load' :
                   'Load Now'}
                </button>
              </div>
            </div>
            ...
            <div className="mt-8 bg-green-50 border border-green-200 rounded-xl p-6">
              <h3 className="text-lg font-semibold text-green-900 mb-2">
                ⚡ {currentLanguage === 'ko' ? '빠른 충전 혜택' :
                    currentLanguage === 'tl' ? 'Instant Load Benefits' :
                    'Instant Load Benefits'}
              </h3>
              <ul className="text-sm text-green-800 space-y-1">
                <li>• {currentLanguage === 'ko' ? '즉시 충전 완료 (평균 30초)' :
                        currentLanguage === 'tl' ? 'Instant load (average 30 seconds)' :
                        'Instant load completion (average 30 seconds)'}</li>
                <li>• {currentLanguage === 'ko' ? '24시간 연중무휴 서비스' :
                        currentLanguage === 'tl' ? '24/7 na serbisyo' :
                        '24/7 service availability'}</li>
                <li>• {currentLanguage === 'ko' ? '모든 프로모와 데이터 플랜 지원' :
                        currentLanguage === 'tl' ? 'Suportado ang lahat ng promo at data plans' :
